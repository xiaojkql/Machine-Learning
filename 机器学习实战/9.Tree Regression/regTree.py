...
def regLeaf(dataSet):
    return mean(dataSet[:, -1]) # 计算目标函数的均值，即最后一行的均值
...
def regErr(dataSet):
    ...
    return var(dataSet[:, -1]) * shape(dataSet)[0] # 计算混乱程度 用总方差来表示混乱程度


# 1.用最佳方式切分数据集
# 2.生成相应的叶节点
def chooseBestSplit(dataSet, leafType=regLeaf, errType=regErr, ops=(1, 4)): # 作为两个函数对象进行传入
...
def prune(tree, testData):
    ...
    if isTree(tree['left']):
        tree['left'] = prune(tree['left'], lSet) # 传入的testdata集合已经改变了
    ...
    if not isTree(tree['left']) and not isTree(tree['right']):
        ...
        treeMean = (tree['left'] + tree['right'])/2.0 # tree预测的值
